...
protected:
	...
	struct RegressionProxy{
		template<class Iter, typename = decltype(*std::declval<Iter&>(), void(), ++std::declval<Iter&>(), void())>
		InputDataPtr Regression(Iter input_begin, Iter input_end, OutputParamType_ teacher_value) const{
	...
	struct ClassificationProxy{
		template<class Iter, typename = decltype(*std::declval<Iter&>(), void(), ++std::declval<Iter&>(), void())>
		InputDataPtr Classification(Iter input_begin, Iter input_end, int teacher_label) const{
			...
			if (std::is_same<typename OutputInfo_::layer_type<OutputInfo_>, MultiClassClassificationLayer<OutputInfo_>>::value){
				teacher = DataFormat<InputInfo_, OutputInfo_>::ConvertBinaryVector(label);
			}
	...
	//教師信号なし (兼テストデータ)
	struct UnsupervisedProxy{
		template<class Iter, typename = decltype(*std::declval<Iter&>(), void(), ++std::declval<Iter&>(), void())>
		InputDataPtr Unsupervised(Iter input_begin, Iter input_end) const{
...
public:
	...
	static auto ConvertBinaryVector(uint class_label) ->OutputArrayType_{
