...
public:
	...
	std::shared_ptr<InputData> MakeInputData(Iter1 test_begin, Iter1 test_end) const{
		std::array<typename OutputInfo_::type, OutputInfo_::dim> teacher;
		return std::make_shared<InputData>(test_begin, test_end, teacher.begin(), teacher.end(), true);
	}
};
